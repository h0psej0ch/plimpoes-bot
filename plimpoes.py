...
plimpoesEmbed.set_image(
  url=
  "https://media.discordapp.net/stickers/1062405909910933565.webp?size=160"
)
...
class Music(commands.Cog):
    ...
    async def _queue(self, ctx: commands.Context, *, page: int = 1):
        ...
        musicpoesid = 1075177387051991140
...
bot = commands.Bot('=', description='new Palacce music bot.', intents=intents)
...
async def on_message(message):
  ...
  if message.author != bot.user:
    if str(message.channel) == "plimpoes-appreciation-channel":
    ...
    elif str(message.channel) == "musicpoes":
      if message.content.startswith('='):
        ...
        async for _ in message.channel.history(limit=None):
          count += 1
        limit = count - 1
        await chan.purge(limit=limit)
        await bot.process_commands(message)
      elif message.content.startswith("join"):
        ctx = await bot.get_context(message)
        await ctx.invoke(Music._queue(ctx))
        
      else:
        ctx = await bot.get_context(message)
        await Music._queue()

@bot.tree.command(name="plimpoes",
                  description="Rates how much plimpoes you are :hot_face:")
